...
{name}!""".format(n = n,name2 = name2, ame = ame, name = name, ame2 = ame2, screen_name = screen_name)
    ...
    if len(rhyme) > 140:
            ...
            rhyme = "@{screen_name} you requested {name} as a rhyme but it's too long for twitter. I DMed you!".format(name = name, screen_name = screen_name)
    return rhyme
...
def main():
    ...
    for idx, item in enumerate(mentions):
        ...
        if status != 0:
           try:
              twitter.statuses.update(status=status,in_reply_to_status_id = item['id'])
